jQuery(document).ready(function ($) {
    ...
    noticePanel.data("auto-show") &&
        ...
        setTimeout(function () {
            noticePanel.fadeIn(300), (isPanelVisible = !0);
        }, 500),
        ...
        $(document).on("click", "#cpfm_remove_notice", function (e) {
            e.preventDefault(), noticePanel.fadeOut(300), (isPanelVisible = !1);
        }),
        $(document).on("click", ".opt-in-yes, .opt-in-no", function (e) {
            ...
            const button = $(this),
                ...
                optIn = button.val(),
                noticeItem = button.closest(".notice-item");
            $.post(
                adminNotice.ajaxurl,
                { action: "cpfm_handle_opt_in", nonce: adminNotice.nonce, category: category, opt_in: optIn },
                function (response) {
                    response.success
                        ? noticeItem.slideUp(300, function () {
                              noticeItem.remove();
                              const remainingNotices = $("#cpfNoticePanel .notice-item.unread"),
                                  count = remainingNotices.length;
                ...
                jqXHR.responseJSON &&
                    ...
                    alert(errorMessage),
                    console.error("AJAX request failed:", textStatus, jqXHR);
            });
        }),
